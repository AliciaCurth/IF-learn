"""
Author: Alicia Curth
Utils to create the IF-learner simulation (simulation study 1) results in Curth, Alaa and van der
Schaar (2020)
"""
import numpy as np
import pandas as pd
...
from iflearn.simulation_utils.base import BASE_BASELINE_MODEL, ModelCaller
...
def eval_range_n(estimator, range_n, repeats=N_REPEATS_BASE, n_test=N_TEST_BASE, d=1,
                 ...
                 propensity_model=None, error_model=None, setting=CATE_NAME,
                 pre_dispatch='2*n_jobs', n_jobs=1, verbose=0):
    ...
    for n in range_n:
        ...
        scores = eval_setting_repeat(estimator=estimator, n_train=n, te_estimator=te_estimator,
                                     repeats=repeats, n_test=n_test, setting=setting,
...
def eval_range_d(estimator, range_d, propensity_model=None, setting=CATE_NAME,
    ...
    for d in range_d:
        ...
        scores = eval_setting_repeat(estimator=estimator, n_train=n_train, setting=setting,
...
def eval_range_bias(estimator, range_b, propensity_model=None, repeats=N_REPEATS_BASE,
                    n_test=N_TEST_BASE, n_train=N_TRAIN_BASE, setting=CATE_NAME,
                    d=1, te_function=None, binary_y=False, te_estimator=None,
                    baseline_model=None, error_model=None,
                    pre_dispatch='2*n_jobs', n_jobs=1, verbose=0):

    if propensity_model is None:
        propensity_model = propensity_model_2

    resultframe = pd.DataFrame(columns=['t_mean', 'ot_mean', 'if_mean', 'oif_mean', 't_sd',
                                        'ot_sd', 'if_sd', 'oif_sd'])

    for b in range_b:
        selection_model = ModelCaller(propensity_model, args={'b': b})
        print('Bias with parameter: {}'.format(b))
        scores = eval_setting_repeat(estimator=estimator, n_train=n_train, setting=setting,
                                     repeats=repeats, n_test=n_test, te_estimator=te_estimator,
